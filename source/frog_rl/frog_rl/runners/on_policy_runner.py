...
import frog_rl
from frog_rl.algorithms import PPO, Distillation
...
from frog_rl.utils import store_code_state
...
def _migrate_train_cfg(train_cfg: dict) -> None:
    ...
    if "policy" not in train_cfg and "actor" in train_cfg:
        ...
        # Remove the runner-level class_name (e.g. "OnPolicyRunner"); not used here.
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        self.git_status_repos = [frog_rl.__file__]
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from frog_rl.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from frog_rl.utils.wandb_utils import WandbSummaryWriter
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            # -- Log top error timesteps if available
            if "top_error_timesteps" in locs and locs["top_error_timesteps"]:
                ...
                if top_errors.get("error_values"):
                    # Log the error values of top 10 timesteps
