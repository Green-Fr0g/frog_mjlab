...
import frog_rl
from frog_rl.algorithms import AMPPPO
...
from frog_rl.utils import AMPLoader, Normalizer, store_code_state
...
class AmpOnPolicyRunner:
    ...
    def load(self, path: str, load_optimizer: bool = True):
        ...
        if "model_state_dict" in loaded_dict:
            # Native frog_rl format: single combined state dict
            resumed_training = self.alg.policy.load_state_dict(loaded_dict["model_state_dict"])
        elif "actor_state_dict" in loaded_dict:
            # mjlab v5 format: separate actor/critic dicts with mlp.* keys
            ...
            if "critic_state_dict" in loaded_dict:
                for k, v in loaded_dict["critic_state_dict"].items():
                    ...
                    merged[new_key] = v
            # map distribution.std_param -> std
